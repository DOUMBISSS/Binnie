    ...
    try {
      ...
      const res = await fetch(`${API}/api/contact/submit`, {
        ...
        body: JSON.stringify({
          ...
          telephone: formContact.phone || null,
          message:   formContact.message || null,
          type:      "rendez-vous",
          sujet:     "Demande de rappel / RDV",
        }),
      });
      if (!res.ok) throw new Error();
      setFormSent(true);
    } catch {
      setContactErreur("Une erreur est survenue. Veuillez réessayer.");
    } finally {
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, alignItems: "center" }}>
              ...
              {formSent ? (
                ...
                <form onSubmit={handleContactSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  {contactErreur && <p style={{ color:"#dc2626", fontSize:"0.82rem", margin:"0" }}>⚠ {contactErreur}</p>}
                  <button type="submit" disabled={contactLoading} style={{ ...btnPrimary, padding: "12px", opacity: contactLoading ? 0.7 : 1 }}>
                    {contactLoading ? "Envoi…" : "📞 Demander à être rappelé"}
                  </button>
                </form>
              )}
            </div>
