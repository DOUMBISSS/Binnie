...
export default function SimulateurFormation() {
  ...
  const [entreprise, setEntreprise] = useState({
    ...
    secteurAutre: "",    // pour stocker la saisie manuelle si secteur = "Autre"
    nbEmployesAutre: "", // pour stocker la saisie manuelle si nbEmployes = "Autre"
  });
  ...
  // ── Validation étape 1 (avec gestion des champs "Autre") ──
  const canNext = () => {
    if (step === 1) {
      if (!entreprise.nom || !entreprise.emailContact) return false;
      // Secteur : si "Autre" est sélectionné, il faut que le champ texte soit rempli
      if (entreprise.secteur === "Autre" && !entreprise.secteurAutre.trim()) return false;
      // Nombre employés : si "Autre" est sélectionné, il faut que le champ texte soit rempli
  ...
  const handleSendDevis = (e) => {
    e.preventDefault();
    // Ici vous pouvez intégrer l’envoi API et les tracking (gtag, fbq)
    setDevisEnvoye(true);
  };
