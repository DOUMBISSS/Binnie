...
export default function ParcoursEntreprise() {
  ...
  const openModal = (type, offer=null) => { setModal(type); setSelOffer(offer); setSent(false); setErreur(""); setForm({ company:"", sector:"", size:"", employees:"", name:"", role:"", email:"", phone:"", budget:"", message:"", consent:false }); };
  const closeModal = () => { setModal(null); setSent(false); setErreur(""); };

  const handleSubmit = async () => {
    if (!form.company || !form.name || !form.email || !form.phone) return;
    setSub(true); setErreur("");
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5001"}/api/entreprise/submit`, {
        method: "POST",
    ...
    <>
                ...
                <div style={S.heroStatsCard}>
                  ...
                  {[["500+","Entreprises auditées"],["96%","Taux de satisfaction RH"],["24h","Délai de réponse garanti"],["FDFP","Financement disponible"]].map(([num,lbl],i)=>(
                    <div key={i} style={S.heroStatRow}>
                      <span style={{fontFamily:"'DM Serif Display',serif",fontSize:"1.4rem",color:i%2===0?"#f87171":"#93c5fd"}}>{num}</span>
          ...
          <div style={S.inner}>
            ...
            <div ref={planRef} className="pe-plans-grid" style={S.plansGrid}>
              {PLANS.map((p,i)=>(
                <div key={p.id} className="pe-plan" style={{...S.planCard, opacity:planInView?1:0, transform:planInView?"none":"translateY(16px)", transition:`all .5s ease ${i*100}ms`, border:p.popular?`2px solid ${p.color}`:"1.5px solid #e2e8f0", background:p.popular?"#fef2f2":"#fff" }}>
                  ...
                  <h3 style={{fontFamily:"'DM Serif Display',serif",fontSize:"1.2rem",margin:"0 0 8px",fontWeight:400}}>{p.label}</h3>
                  <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:12}}>
                    <span style={{fontFamily:"'DM Serif Display',serif",fontSize:"1.6rem",color:p.color}}>{p.price}</span>
            ...
            <div className="pe-cta-final" style={S.ctaFinal}>
              ...
              <div style={{position:"relative",zIndex:1}}>
                <h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:"clamp(1.5rem,3vw,2.2rem)",color:"#fff",margin:"0 0 12px",fontWeight:400}}>
      ...
      {modal === "devis" && (
        <ModalWrap onClose={closeModal}>
          {sent ? <SuccessView name={form.name} email={form.email} onClose={closeModal} msg={`Merci ${form.name} ! Un expert BET vous contactera sous 24h pour discuter de vos besoins en formation.`}/> : (
            <>
              ...
              <div style={S.modalBody}>
                ...
                <SubmitBtn submitting={submitting} onClick={handleSubmit} label="Envoyer ma demande →"/>
                <p style={{textAlign:"center",fontSize:".73rem",color:"#94a3b8",marginTop:8}}>✓ Gratuit · ✓ Réponse sous 24h · ✓ Sans engagement</p>
              </div>
            </>
          )}
        </ModalWrap>
            ...
            <>
              ...
              <div style={S.modalBody}>
                ...
                <button style={{...S.planBtn,background:"#059669",color:"#fff",border:"none",width:"100%",padding:"13px",fontSize:".96rem",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginTop:4,opacity:submitting?.7:1}} onClick={handleSubmit} disabled={submitting}>
                  {submitting?<><span style={{width:15,height:15,border:"2.5px solid rgba(255,255,255,.35)",borderTopColor:"#fff",borderRadius:"50%",animation:"peSpin .8s linear infinite",display:"inline-block"}}/>Envoi…</>:"🔍 Demander mon audit gratuit"}
                </button>
              </div>
            </>
          )}
...
const SubmitBtn=({submitting,onClick,label})=>(
  <button style={{width:"100%",padding:"13px",background:"linear-gradient(135deg,#1e3a8a,#dc2626)",color:"#fff",border:"none",borderRadius:999,fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:".95rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,opacity:submitting?.7:1}} onClick={onClick} disabled={submitting}>
...
const SuccessView=({name,email,onClose,msg})=>(
  <div style={{textAlign:"center",padding:"36px 28px"}}>
    ...
    <h3 style={{fontFamily:"'DM Serif Display',serif",fontSize:"1.4rem",margin:"0 0 10px",fontWeight:400}}>Demande envoyée !</h3>
    ...
    <button style={{padding:"11px 26px",background:"#1e3a8a",color:"#fff",border:"none",borderRadius:999,fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:".92rem",cursor:"pointer"}} onClick={onClose}>Fermer</button>
  </div>
);
